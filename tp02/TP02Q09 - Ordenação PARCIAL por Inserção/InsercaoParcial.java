/* TP01P2Q10 - Ordenacao por Insercao
 ...
 * */
 public void ordemInsercao (int k){
      int j;
      for (int i = 1; i < n; i++)
      {
         Instituicao tmp = array[i];
         if (i > k) {
            j = k;
         } else { j = i-1; }
         boolean stop = false;
        comparacoes++;
        while ((j>= 0 && ((array[j]).getCodigoMantenedora() >= tmp.getCodigoMantenedora())) && !stop) {
         //testar se o cod mantedora sao iguais
          comparacoes = comparacoes+2;
          if (((array[j]).getCodigoMantenedora() ==
                      tmp.getCodigoMantenedora())) {
               comparacoes++;
             if (tmp.getCodigo() < array[j].getCodigo ()) {
               array [j+1] = array [j];
               movimentacoes++;
               j--;
             }//fim if
 ...
 //main
  public static void main (String [] args)
  {
   ...
   try {
    InsercaoParcial lista = new InsercaoParcial (200);
   //abrir arquivo
    FileWriter arquivo = new FileWriter ("628510_insercao.txt");
